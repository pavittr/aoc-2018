...
[1518-11-05 00:55] wakes up`
	...
	findGuard := func(sequence string) (int, int) {
		dateMessageMatcher := regexp.MustCompile(`\[(?P<date>\d+-\d+-\d+ \d+:\d+)\] (?P<message>.*)`)
		eventLog := make([]LogLine, 0)
		for _, line := range strings.Split(sequence, "\n") {

			if !dateMessageMatcher.MatchString(line) {
				panic(fmt.Sprintf("Messgae is unreadable. line %+v", line))
			}

			groups := dateMessageMatcher.FindStringSubmatch(line)
			timestampStr := groups[1]
			timestamp, err := time.Parse(`2006-01-02 15:04`, timestampStr)
			if err != nil {
				panic(fmt.Sprintf("Error on line %+v, err %+v", line, err))
			}
			messageStr := groups[2]
			message := Message{}
			switch {
			case strings.HasPrefix(messageStr, "Guard"):
				message.Type = "Guard"
				messageID, err := strconv.Atoi(strings.Split(strings.Split(messageStr, "#")[1], " ")[0])
				if err != nil {
					panic("Bad split")
				}
				message.ID = messageID

			case strings.HasPrefix(messageStr, "falls"):
				message.Type = "asleep"
			case strings.HasPrefix(messageStr, "wakes"):
				message.Type = "awake"
			default:
				panic(fmt.Sprintf("BAd line %+v\n", line))
			}

			eventLog = append(eventLog, LogLine{timestamp, message})
		}
		sort.Slice(eventLog, func(i, j int) bool { return eventLog[i].Timestamp.Before(eventLog[j].Timestamp) })

		sortedLog := make([]LogLine, 0)
		currentGuard := -1
		for _, line := range eventLog {
			if line.Message.Type == "Guard" {
				currentGuard = line.Message.ID
			}
			if line.Timestamp.Hour() == 0 && line.Message.Type != "Guard" {
				sortedLog = append(sortedLog, LogLine{Timestamp: line.Timestamp, Message: Message{Type: line.Message.Type, ID: currentGuard}})
			}
		}
		...
		max := 0
		maxGuard := -1
		for guardId, count := range guardGaps {
			if count > max {
				max = count
				maxGuard = guardId
			}
		...
		for i := 0; i < len(testLog); i = i + 2 {
			if testLog[i].Message.ID == maxGuard {
				for minute := testLog[i].Timestamp.Minute(); minute < testLog[i+1].Timestamp.Minute(); minute++ {
	...
	testGuard, testMinute := findGuard(testInput)
	//§	assert.Equal(t, 24, testGuard.MostCommonMinute)
	assert.Equal(t, 10, testGuard)
	assert.Equal(t, 24, testMinute)
	puzzleGuard, puzzleInput := findGuard(day4Input)
	//§	assert.Equal(t, 24, testGuard.MostCommonMinute)
	assert.Equal(t, 1549, puzzleGuard)
	assert.Equal(t, 41, puzzleInput)
	assert.Equal(t, 63509, puzzleGuard*puzzleInput)
...
[1518-11-05 00:55] wakes up`
	...
	findGuard := func(sequence string) GuardMinuteKey {
		dateMessageMatcher := regexp.MustCompile(`\[(?P<date>\d+-\d+-\d+ \d+:\d+)\] (?P<message>.*)`)
		eventLog := make([]LogLine, 0)
		for _, line := range strings.Split(sequence, "\n") {
		...
		for i := 0; i < len(testLog); i = i + 2 {
			for minute := testLog[i].Timestamp.Minute(); minute < testLog[i+1].Timestamp.Minute(); minute++ {
				...
				minuteMap[GuardMinuteKey{testLog[i].Message.ID, minute}] += 1
			}
		}
		//
		maxMinuteStruct := GuardMinuteKey{}
		maxMinuteCount := -1
		for minuteStruct, count := range minuteMap {
			if count > maxMinuteCount {
				maxMinuteStruct = minuteStruct
				maxMinuteCount = count
			}
		}
		return maxMinuteStruct
	}

	testGuard := findGuard(testInput)
	assert.Equal(t, 99, testGuard.Guard)
...
var day4Input = `[1518-09-29 00:35] falls asleep
