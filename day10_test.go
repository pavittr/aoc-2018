...
func (s *Sky) paintWithStars(w io.Writer, loop int) { //Geddit
	bottomX, bottomY, topX, topY := s.findRange()
	_, err := fmt.Fprintf(w, "Loop %d. Range is %d, %d, %d, %d\n", loop, bottomX, bottomY, topX, topY)
	check(err)
	coordMap := s.starsToCoords()
	for y := bottomY; y <= topY; y++ {
		for x := bottomX; x <= topX; x++ {
			s.PrintStar(w, x, y, coordMap)
		}
		_, err := fmt.Fprintf(w, "\n")
		check(err)
	}
...
func (s *Sky) starsToCoords() map[Coord]bool {
	returnedMap := make(map[Coord]bool)
	for _, star := range s.Stars {
		returnedMap[Coord{star.X, star.Y}] = true
	}
	return returnedMap
}

func (s *Sky) PrintStar(w io.Writer, xpos, ypos int, coordMap map[Coord]bool) {
	if coordMap[Coord{xpos, ypos}] {
		_, err := fmt.Fprintf(w, "#")
		check(err)
	} else {
		_, err := fmt.Fprintf(w, ".")
...
func (s *Sky) findRange() (int, int, int, int) {
	bottomX, bottomY, topX, topY := 0, 0, 0, 0
	for _, coord := range s.Stars {
		if coord.X < bottomX {
			bottomX = coord.X
		}
		if coord.X > topX {
...
func (s *Sky) addStar(desc string) {

	starParser := regexp.MustCompile(`position=<\s*(?P<xpos>-?\d+),\s+(?P<ypos>-?\d+)> velocity=<\s*(?P<xvel>-?\d+),\s+(?P<yvel>-?\d+)>`)

	if !starParser.MatchString(desc) {
		panic(fmt.Sprintf("Message is unreadable. line %+v", desc))
	}

	groups := starParser.FindStringSubmatch(desc)
	xpos, err := strconv.Atoi(groups[1])
	if err != nil {
		panic(fmt.Sprintf("BAd err on group %v (%v) on line %v. Err %v", 1, groups[1], desc, err))
	}
	ypos, err := strconv.Atoi(groups[2])
	if err != nil {
		panic(fmt.Sprintf("BAd err on group %v (%v) on line %v. Err %v", 2, groups[2], desc, err))
	}
	xvel, err := strconv.Atoi(groups[3])
	if err != nil {
		panic(fmt.Sprintf("BAd err on group %v (%v) on line %v. Err %v", 3, groups[3], desc, err))
	}
	yvel, err := strconv.Atoi(groups[4])
	if err != nil {
		panic(fmt.Sprintf("BAd err on group %v (%v) on line %v. Err %v", 4, groups[4], desc, err))
	}
	s.Stars = append(s.Stars, &Star{xpos, ypos, xvel, yvel})
...
position=<-3,  6> velocity=< 2, -1>`
	stars := make([]*Star, 0)
	testSky := &Sky{Stars: stars}
	for _, line := range strings.Split(testInput, "\n") {
		testSky.addStar(line)

	}
	assert.Equal(t, 31, len(testSky.Stars))
	...
	w := bufio.NewWriter(f)
	writeHeader(w)
	testSky.paintWithStars(w, 0)
	fmt.Printf("Moving to t=1\n")
	testSky.moveOn()
	testSky.paintWithStars(w, 1)
	fmt.Printf("Moving to t=2\n")
	testSky.moveOn()
	testSky.paintWithStars(w, 2)
	fmt.Printf("Moving to t=3\n")
	testSky.moveOn()
	testSky.paintWithStars(w, 3)

	stars = make([]*Star, 0)
	puzzleSky := &Sky{stars}
	for _, line := range strings.Split(day10Input, "\n") {
		puzzleSky.addStar(line)
	}
	for i := 0; i < 2000000; i++ {

		_, bottomY, _, topY := puzzleSky.findRange()
		if topY-bottomY < 200 {
			puzzleSky.paintWithStars(w, i)
		}
		puzzleSky.moveOn()
	}
	w.Flush()
...
var day10Input = `position=<-10351, -10360> velocity=< 1,  1>
