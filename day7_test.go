...
func TestDay7Part1(t *testing.T) {
	...
	stepOrder := func(input string) string {
		...
		actionedSteps := make(map[string]bool)
		for _, line := range strings.Split(input, "\n") {
			...
			dependency := groups[1]
			stepLetter := groups[2]
			fmt.Printf("Line has dep %v and step %v\n", dependency, stepLetter)
			...
			// Think this is unessessary?
			actionedSteps[dependency] = false
		}

		for step, dependencies := range dependencies {
			fmt.Printf("Step %v, dependencies: %+v\n", step, dependencies)
		}
		return ""
	}
